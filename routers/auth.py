...
# --- NOUVEAU SYSTÈME DE HACHAGE AVEC BCRYPT DIRECT ---
...
def verify_otp(data: OTPVerify):
    driver = get_db()
    with driver.session() as session:
        record = session.run("MATCH (u:User {email: $email}) RETURN u.otp AS otp, u.is_verified AS is_verified", email=data.email).single()
